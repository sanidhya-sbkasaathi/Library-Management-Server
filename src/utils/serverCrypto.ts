/**
 * MANAGEMENT SERVER CRYPTOGRAPHIC SIGNING ENGINE (Control Plane Authority)
 * 
 * Algorithm: Ed25519 (Asymmetric Digital Signatures)
 * RFC 8032 / Web Cryptography API Native Hardware Implementation
 * 
 * SECURITY RULES:
 * 1. The Private Key is stored ONLY on this Management Server.
 * 2. Client applications (Library App / APK) only receive the PUBLIC verification key.
 * 3. Signatures cover canonicalized JSON payloads (deterministic key ordering).
 * 4. Distinct credential types: 'LIBRARY_OWNER' vs 'LIBRARY_ROLE'.
 */
